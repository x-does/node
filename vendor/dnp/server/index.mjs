...
export function createDefinitelyNotPongServer(options = {}) {
  ...
  app.get(route(basePath, '/join/:code'), (req, res) => {
    res.sendFile(path.join(clientDir, 'join.html'), (err) => {
      if (err) {
        res.type('html').send(`<!doctype html><html><head><title>DefinitelyNotPong</title></head><body><main><h1>DefinitelyNotPong</h1><p>Join code: <strong>${String(req.params.code).toUpperCase()}</strong></p></main></body></html>`);
      }
    });
  });
