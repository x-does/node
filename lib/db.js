...
async function loadScopedLeadMetrics(eventKey, scopeClause = '', scopeParams = []) {
...
export async function getLeadMetrics(eventKey) {
  ...
  const last24h = await loadScopedLeadMetrics(eventKey, 'AND created_at >= (UTC_TIMESTAMP() - INTERVAL 24 HOUR)');
  const last60m = await loadScopedLeadMetrics(eventKey, 'AND created_at >= (UTC_TIMESTAMP() - INTERVAL 60 MINUTE)');
