...
export async function getLeadMetrics(eventKey) {
  ...
  const internalSources = bySource.filter((item) => isInternalSource(item.source));
  const externalSources = bySource.filter((item) => !isInternalSource(item.source));
  ...
  const [externalUniqueRows] = await getPool().query(
    `SELECT COUNT(DISTINCT COALESCE(ip_hash, user_agent, CONCAT('event-', id))) AS unique_count
     FROM openclaw_lead_events
     WHERE event_key = ?
       AND source NOT LIKE 'deploy_probe%'
       AND source NOT LIKE 'verify_%'
       AND source NOT LIKE 'internal_%'
       AND source NOT LIKE 'monitor_%'`,
    [eventKey]
  );

  const externalUnique = Number(externalUniqueRows?.[0]?.unique_count || 0);
